...
nuke::nuke(const Vector2f& position, Vector2f direction, float angle) : aoe_projectile(position, 4, direction, angle)
{
	...
	set_shape_points(shape_projectile_, 10, 15);
...
void nuke::update(float delta_t)
{
...
void nuke::explode(Vector2f explosion_area)
{
	hasExploded = true;
	time_since_created_ = 0;
	projectile_damage_ = 500000;
	max_force_ = 1000;
	velocity_ = Vector2f(0, 0);
	max_time_ = 0.1f;

	set_shape_points(shape_projectile_, 250, 250);
	rotation_angle_ = 45;
	proj_color_ = Color::Yellow;
	proj_color_.a = 200;
	shape_projectile_.setFillColor(proj_color_);
}
